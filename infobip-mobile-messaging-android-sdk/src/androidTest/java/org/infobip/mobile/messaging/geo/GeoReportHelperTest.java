...
public class GeoReportHelperTest extends MobileMessagingTestCase {
    ...
    public void test_should_update_list_of_inactive_campaigns_based_on_report_result() {
        ...
        // When
        Set<String> ids = GeoReportHelper.getAndUpdateInactiveCampaigns(mobileMessagingCore, new GeoReportingResult(reportResponse));

        // Then
        assertTrue(mobileMessagingCore.getSuspendedCampaignIds().contains("campaignId1"));
        assertTrue(ids.contains("campaignId1"));
        assertTrue(mobileMessagingCore.getFinishedCampaignIds().contains("campaignId2"));
